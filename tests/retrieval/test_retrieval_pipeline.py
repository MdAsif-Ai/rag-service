...
async def test_dense_and_sparse_receive_precomputed_vectors(pipeline, mock_components):
    ...
    args, kwargs = dense.retrieve.call_args
    assert kwargs["course_ids"] == ["course1"]
    assert args[0] == [0.1, 0.2]  # Dense vector passed positionally
    
    sparse.retrieve.assert_called_once()
    args, kwargs = sparse.retrieve.call_args
    assert kwargs["course_ids"] == ["course1"]
    assert args[0] == {1: 0.5}  # Sparse vector passed positionally
...
async def test_pipeline_flow_and_top_k(pipeline, mock_components):
    ...
    with patch('app.retrieval.pipeline.get_settings') as mock_settings:
        mock_settings.return_value.DENSE_TOP_K = 50
        mock_settings.return_value.SPARSE_TOP_K = 50
        mock_settings.return_value.FUSION_TOP_K = 10
        mock_settings.return_value.FINAL_TOP_K = 5
...
@pytest.mark.asyncio
async def test_latency_metrics_are_generated(pipeline, mock_components):
    _, dense, sparse, fusion, reranker = mock_components
    c1 = RetrievalCandidate(chunk_id="1", document_id="d", course_id="c", filename="f", content="a", chunk_index=0)
    dense.retrieve.return_value = [c1]
    sparse.retrieve.return_value = []
    fusion.fuse.return_value = [c1]
    reranker.rerank.return_value = [c1]
    ...
    with patch('app.retrieval.pipeline.get_settings') as mock_settings:
        ...
        response = await pipeline.retrieve("test query", ["course1"], 1)
        
        assert response.timings.embedding_latency_ms >= 0.0
        assert response.timings.dense_latency_ms >= 0.0
        assert response.timings.sparse_latency_ms >= 0.0
        assert response.timings.fusion_latency_ms >= 0.0
        assert response.timings.reranking_latency_ms >= 0.0
        assert response.timings.total_latency_ms >= 0.0
        assert response.timings.query_hash is not None