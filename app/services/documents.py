...
class DocumentService:
    ...
    async def get_document(document_id: uuid.UUID) -> DocumentDetail:
        ...
        try:
            ...
            doc_data = cast(Dict[str, Any], doc_res.data[0]).copy()
            ...
            latest_job = None
            if job_res.data:
                job_data = cast(Dict[str, Any], job_res.data[0]).copy()
    ...
    async def reindex_document(document_id: uuid.UUID) -> IngestionResponse:
        ...
        try:
            ...
            doc_data = cast(Dict[str, Any], doc_res.data[0])
            ...
            supabase.table("ingestion_jobs").insert({
                "id": job_id, "document_id": str(document_id), "status": JobStatus.QUEUED, "stage": JobStage.UPLOADED
            }).execute()
            supabase.table("documents").update({"status": "PROCESSING"}).eq("id", str(document_id)).execute()
            
            ingest_document.delay(str(document_id), job_id)
            return IngestionResponse(document_id=document_id, job_id=uuid.UUID(job_id), status=JobStatus.QUEUED)
        except HTTPException:
            raise
        except Exception as e:
